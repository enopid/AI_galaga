...
class Galaga(Game):
    def __init__(self):
        ...
        # number of lives in addition to the initial one
        self.num_of_lives = 2
    ...
    def process_input(self):
        ...
        if self.key_down(pg.K_LEFT) or self.AI.move_state==-1:
            self.direction = -1

        if self.key_down(pg.K_RIGHT) or self.AI.move_state==1:
            self.direction = 1

        if self.key_pressed(pg.K_LCTRL) or self.AI.shoot_state==1:
            self.fire = 1
            self.AI.shoot_state==0
        ...
        if self.key_pressed(pg.K_s):
            ...
            pg.image.save(pg.image.frombuffer(image.tobytes(), (width, height), "RGB"), "screenshot.png")
